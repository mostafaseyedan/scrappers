...
async function logout(page: Page) {
  await page.click("#myAccountMenuLink");
  await page.click("#myAccountMenu a[title=\"Logout\"]");
...
async function processRow(
  ...
  const sol = {
    ...
    publishDate,
    site: "merx",
    siteUrl: siteUrl ? "https://www.merx.com" + siteUrl : "",
    siteId,
  };
...
async function scrapeAllSols(
  ...
  await page.goto(
    "https://www.merx.com/private/supplier/solicitations/search?target=init",
    { waitUntil: "domcontentloaded" }
  );
  ...
  const dismiss = page.locator(
    "#supplierUpsellBannerSolicitationSearchDismiss"
  );
...
export async function run(
  ...
  const USER = env.DEV_MERX_USER!;
  const PASS = env.DEV_MERX_PASS!;
  const VENDOR = "merx";
  let results = {};
