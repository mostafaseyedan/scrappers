...
async function test() {
  console.log("🚀 Starting bonfirehub test with Browserbase...");
  ...
  console.log("  BONFIRE_USER:", process.env.BONFIRE_USER ? "✓ Set" : "✗ Missing");
  console.log("  BONFIRE_PASS:", process.env.BONFIRE_PASS ? "✓ Set" : "✗ Missing");
  ...
  try {
    const results = await bonfirehub(
      page,
      {
        ...
        DEV_BONFIRE_USER: process.env.BONFIRE_USER || process.env.DEV_BONFIRE_USER,
        DEV_BONFIRE_PASS: process.env.BONFIRE_PASS || process.env.DEV_BONFIRE_PASS,
