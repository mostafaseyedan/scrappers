...
async function test() {
  console.log("🚀 Starting demandstar test with Browserbase...");
  ...
  console.log("  DEMANDSTAR_USER:", process.env.DEMANDSTAR_USER ? "✓ Set" : "✗ Missing");
  console.log("  DEMANDSTAR_PASS:", process.env.DEMANDSTAR_PASS ? "✓ Set" : "✗ Missing");
  ...
  try {
    const results = await demandstar(
      page,
      {
        ...
        DEV_DEMANDSTAR_USER: process.env.DEMANDSTAR_USER || process.env.DEV_DEMANDSTAR_USER,
        DEV_DEMANDSTAR_PASS: process.env.DEMANDSTAR_PASS || process.env.DEV_DEMANDSTAR_PASS,
