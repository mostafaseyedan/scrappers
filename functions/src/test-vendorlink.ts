...
async function test() {
  console.log("🚀 Starting vendorlink test with Browserbase...");
  ...
  console.log("  VENDORLINK_USER:", process.env.VENDORLINK_USER ? "✓ Set" : "✗ Missing");
  console.log("  VENDORLINK_PASS:", process.env.VENDORLINK_PASS ? "✓ Set" : "✗ Missing");
  ...
  try {
    const results = await vendorlink(
      page,
      {
        ...
        DEV_VENDORLINK_USER: process.env.VENDORLINK_USER || process.env.DEV_VENDORLINK_USER,
        DEV_VENDORLINK_PASS: process.env.VENDORLINK_PASS || process.env.DEV_VENDORLINK_PASS,
