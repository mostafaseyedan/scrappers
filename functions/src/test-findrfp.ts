...
async function test() {
  console.log("🚀 Starting findrfp test with Browserbase...");
  ...
  console.log("  FINDRFP_USER:", process.env.FINDRFP_USER ? "✓ Set" : "✗ Missing");
  console.log("  FINDRFP_PASS:", process.env.FINDRFP_PASS ? "✓ Set" : "✗ Missing");
  ...
  try {
    const results = await findrfp(
      page,
      {
        ...
        DEV_FINDRFP_USER: process.env.FINDRFP_USER || process.env.DEV_FINDRFP_USER,
        DEV_FINDRFP_PASS: process.env.FINDRFP_PASS || process.env.DEV_FINDRFP_PASS,
