...
async function test() {
  console.log("🚀 Starting mygovwatch test with Browserbase...");
  ...
  console.log("  MYGOVWATCH_USER:", process.env.MYGOVWATCH_USER ? "✓ Set" : "✗ Missing");
  console.log("  MYGOVWATCH_PASS:", process.env.MYGOVWATCH_PASS ? "✓ Set" : "✗ Missing");
  ...
  try {
    const results = await mygovwatch(
      page,
      {
        ...
        DEV_MYGOVWATCH_USER: process.env.MYGOVWATCH_USER || process.env.DEV_MYGOVWATCH_USER,
        DEV_MYGOVWATCH_PASS: process.env.MYGOVWATCH_PASS || process.env.DEV_MYGOVWATCH_PASS,
