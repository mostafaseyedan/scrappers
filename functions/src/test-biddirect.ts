...
async function test() {
  console.log("🚀 Starting biddirect test with Browserbase...");
  ...
  console.log("  BIDDIRECT_USER:", process.env.BIDDIRECT_USER ? "✓ Set" : "✗ Missing");
  console.log("  BIDDIRECT_PASS:", process.env.BIDDIRECT_PASS ? "✓ Set" : "✗ Missing");
  ...
  try {
    const results = await biddirect(
      page,
      {
        ...
        DEV_BIDDIRECT_USER: process.env.BIDDIRECT_USER || process.env.DEV_BIDDIRECT_USER,
        DEV_BIDDIRECT_PASS: process.env.BIDDIRECT_PASS || process.env.DEV_BIDDIRECT_PASS,
