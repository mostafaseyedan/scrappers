...
async function test() {
  console.log("🚀 Starting highergov test with Browserbase...");
  ...
  console.log("  HIGHERGOV_USER:", process.env.HIGHERGOV_USER ? "✓ Set" : "✗ Missing");
  console.log("  HIGHERGOV_PASS:", process.env.HIGHERGOV_PASS ? "✓ Set" : "✗ Missing");
  ...
  try {
    const results = await highergov(
      page,
      {
        ...
        DEV_HIGHERGOV_USER: process.env.HIGHERGOV_USER || process.env.DEV_HIGHERGOV_USER,
        DEV_HIGHERGOV_PASS: process.env.HIGHERGOV_PASS || process.env.DEV_HIGHERGOV_PASS,
