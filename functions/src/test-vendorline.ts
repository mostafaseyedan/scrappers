...
async function test() {
  console.log("🚀 Starting vendorline test with Browserbase...");
  ...
  console.log("  VENDORLINE_USER:", process.env.VENDORLINE_USER ? "✓ Set" : "✗ Missing");
  console.log("  VENDORLINE_PASS:", process.env.VENDORLINE_PASS ? "✓ Set" : "✗ Missing");
  ...
  try {
    const results = await vendorline(
      page,
      {
        ...
        DEV_VENDORLINE_USER: process.env.VENDORLINE_USER || process.env.DEV_VENDORLINE_USER,
        DEV_VENDORLINE_PASS: process.env.VENDORLINE_PASS || process.env.DEV_VENDORLINE_PASS,
